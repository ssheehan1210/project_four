...
class App extends Component {
  ...
  creatingNewDungeonDragonSheet = () => {
    const state = this.state;
    const currentPoster = state.currentUsername;
    let posterDungeonDragonData = '';
    for (let f = 0; f < state.userData.length; f++) {
      if (state.userData[f].name === currentPoster) {
        const posterData = state.userData.find(x => x.name === currentPoster);
        const posterId = posterData.id;
        for (let g = 0; g < state.postData.length; g++) {
          if (state.postData[g].user_id === posterId && state.postData[g].dungeons_and_dragons === true) {
    ...
    if (posterDungeonDragonData === '') {
      console.log('No Dungeons and Dragons sheet was found for ' + currentPoster + '. Now creating a blank sheet.');
      this.createNewPost(state.currentUsername, true, state.currentId);
    } else {
